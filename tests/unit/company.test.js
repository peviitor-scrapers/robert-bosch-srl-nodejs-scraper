...
const BOSCH_ANAF_RECORD = {
  cui: 5541546,
  name: 'ROBERT BOSCH SRL',
  address: 'MUNICIPIUL BUCUREŞTI, SECTOR 1, STR HORIA MĂCELARIU, NR.30-34',
  caenCode: '2931',
  inactive: false,
  vatRegistered: true,
  eFacturaRegistered: false,
  headquartersAddress: { locality: 'Sector 1 Mun. Bucureşti' }
};
...
describe('company.js', () => {
  ...
  describe('getCompanyData (no cache)', () => {
    it('should fetch Robert Bosch via direct CIF lookup and return company data', async () => {
      mockFetch.mockResolvedValueOnce(anafCompanyResponse(BOSCH_ANAF_RECORD));

      const result = await company.getCompanyData();

      expect(result).toHaveProperty('company', 'ROBERT BOSCH SRL');
      expect(result).toHaveProperty('cif', '5541546');
      expect(result).toHaveProperty('active', true);
      expect(result).toHaveProperty('anafData');
      expect(result.anafData.name).toBe('ROBERT BOSCH SRL');
    });
    ...
    it('should throw when ANAF returns no company name', async () => {
      mockFetch.mockResolvedValueOnce(anafCompanyResponse({ cui: 5541546, name: null }));
  ...
  describe('getCompanyData (with cache)', () => {
    ...
    it('should use cached company data when available', async () => {
      ...
      expect(result.cif).toBe('5541546');
  ...
  describe('validateAndGetCompany', () => {
    ...
    it('should return company data with status active', async () => {
      mockFetch
        .mockResolvedValueOnce(anafCompanyResponse(BOSCH_ANAF_RECORD))
        .mockResolvedValueOnce(solrResponse(5, [
          { url: 'https://test.com/1', title: 'Job 1' },
          { url: 'https://test.com/2', title: 'Job 2' }
        ]))
        .mockResolvedValueOnce(peviitorResponse([{ company: 'ROBERT BOSCH SRL' }]));
    ...
    // ROBERT BOSCH e activă — testul inactive se rulează doar dacă firma e inactivă
    if (BOSCH_ANAF_RECORD.inactive) {
      it('should return inactive status when company is inactive', async () => {
        const inactiveRecord = { ...BOSCH_ANAF_RECORD, inactive: true };
