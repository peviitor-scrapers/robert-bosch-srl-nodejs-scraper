...
This scraper extracts job listings from the SmartRecruiters API (Bosch Group) and imports them to peviitor.ro.

Target: https://jobs.smartrecruiters.com/BoschGroup (API: `https://api.smartrecruiters.com/v1/companies/BoschGroup/postings`)
...
1. **Start with brand** - We know the brand ("Robert Bosch")
...
8. **Scrape new jobs** - Extract jobs from the SmartRecruiters API (Romania only)
...
> **Important**: Scraper does NOT delete jobs from other sources (ANOFM, etc). It only upserts SmartRecruiters jobs. Existing jobs are preserved.
...
3. **Scrape jobs** - Extract jobs from the SmartRecruiters API (Romania only)
...
company.js (validate company)
    ...
    │
    ▼ (if active)
scrape SmartRecruiters API (jobs for Romania, paginated)
    │
    ▼
...
| `scraper/config/scraper.json` | Scraper-specific target config (SmartRecruiters API URL, page size, max pages) |
...
- **SmartRecruiters API**: `https://api.smartrecruiters.com/v1/companies/BoschGroup/postings` — GET with query params (`country`, `limit`, `offset`)
